...
if [[ ${#read_files[@]} -eq 0 ]]; then
  echo "run_metaflye_pacbio.sh: missing --reads" >&2
  exit 1
fi
...
# Run metaFlye (PacBio)
if ! flye --pacbio-raw "${read_files[@]}" \
          --threads "${cpus}" --scaffold --out-dir '.' --meta; then
  fail "metaFlye (PacBio): assembly failed"
fi

# Map reads back with minimap2 + samtools
if ! ( minimap2 -ax map-pb -t "${cpus}" assembly.fasta "${read_files[@]}" \
