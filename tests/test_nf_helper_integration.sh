...
assert_path_exists "$REPO_ROOT/external/nf-helper/conf/sites/raven.config"
...
assert_file_contains "$REPO_ROOT/conf/raven.config" "external/nf-helper/conf/sites/raven.config"
...
assert_download_srr_runs_local "$REPO_ROOT/conf/raven.config"
...
assert_file_contains "$REPO_ROOT/conf/raven.config" "raven {"
assert_file_contains "$REPO_ROOT/conf/raven.config" "withName: SINGLEM"
assert_file_contains "$REPO_ROOT/conf/raven.config" "clusterOptions = '--export=ALL --ntasks-per-core=2'"
assert_file_contains "$REPO_ROOT/conf/raven.config" "cpus = { Math.min(64 * task.attempt, params.raven_max_cpus_ht as int) }"
assert_file_contains "$REPO_ROOT/conf/raven.config" "cpus: params.raven_max_cpus_ht as int"
assert_file_contains "$REPO_ROOT/conf/raven.config" "withName: 'COMEBIN_GPU|VAMB_GPU|LORBIN_GPU' {"
assert_file_contains "$REPO_ROOT/conf/raven.config" "cpus = { 18 * (params.gpus as int) }"
assert_file_contains "$REPO_ROOT/conf/raven.config" "memory: 125000.MB * (params.gpus as int)"
assert_file_contains "$REPO_ROOT/conf/raven.config" "withName: 'DOWNLOAD_SRA_METADATA|RESOLVE_SRR_METADATA|SANDPIPER' {"
assert_file_contains "$REPO_ROOT/external/nf-helper/conf/sites/raven.config" "withLabel: gpu"
assert_file_contains "$REPO_ROOT/external/nf-helper/conf/sites/raven.config" '$local {'
assert_file_contains "$REPO_ROOT/external/nf-helper/conf/sites/raven.config" "cpus = 2"
assert_file_contains "$REPO_ROOT/external/nf-helper/conf/sites/raven.config" "memory = 32.GB"
assert_file_contains "$REPO_ROOT/external/nf-helper/conf/sites/raven.config" '"--constraint=\"${gpuConstraint}\" --gres=gpu:a100:${gpuCount}"'
assert_file_contains "$REPO_ROOT/external/nf-helper/conf/sites/raven.config" "params.gpu_container_options ?: '--nv'"
...
assert_file_contains "$REPO_ROOT/nextflow.config" "includeConfig \"\${projectDir}/conf/raven.config\""
